(function (root, factory) {
    ...
    converse.initialize = function (settings, operations) {
        ...
        this.default_settings = {
            ...
            password: undefined,
            use_vcards: true,
            domain: '', // 自己所在域名，这里是qchat默认都是ejabhost2
            toDomain: '', // 聊天对象所在域，如果是和qtalk聊天这里会是ejabhost1
        ...
        this.chatBox = {
            ...
            fetchGroupInfo: function () {
                $.ajax({
                    url: converse.http_api_server + '/newapi/muc/get_muc_vcard.qunar?u=' + converse.myId + '&k=' + converse.key,
                    type: 'POST',
                    dataType: "json",
                    data: JSON.stringify([{
            ...
            getVcardForGroup: function (arr) {
                ...
                if (arr.length > 0) {
                    var config = {
                        url: converse.http_api_server + '/newapi/domain/get_vcard_info.qunar?u=' + converse.myId + '&k=' + converse.key,
                        type: 'post',
                        dataType: 'json',
            ...
            fetchHistoryMsg: function (pageSize, strid, ifFirst) {
                ...
                var url = converse.http_api_server + historyApi;
                var params = {
                    from: converse.myId,
                    to: historyStrid,
            ...
            fetchGroupHistoryMsg: function (pageSize, ifFirst) {
                ...
                url = converse.http_api_server + '/package/qtapi/getmucmsgs.qunar';
                $.ajax({
                    url: url,
                    type: 'POST',
                    dataType: "json",
        ...
        this.loginForCookie = function () {
            ...
            if (this.isUCenter) {
                ...
                //匿名登录
                var fromHost = this.getQueryString("fromHost") || "";
                ajaxOption.type = 'post';
                ajaxOption.dataType = 'json';
                ajaxOption.contentType = 'application/json; charset=utf-8';
                ajaxOption.url = converse.http_api_server + 'package/newapi/nck/get_anony_token.qunar';
                data = {
                    plat: fromHost ? fromHost : 'web',//是否从startalk大首页跳转而来，若是，则匿名登录开关--开  startalkWeb
                    version: 10010,
                    gid: "",
                    uuidFlag: ""
                }
            ...
            $.ajax($.extend(ajaxOption, {
                success: function (data) {
                    ...
                    if (self.isAnonymous && !data.data.switchOn) {
    ...
    return {
        ...
        'switchContact': function (strid, virtualId, host) {
            converse.chatBox.stridSpecified(strid, virtualId, host || 'ejabhost2');
        },
