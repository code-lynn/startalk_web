(function() {
    ...
    var imageUpload_url = '//qcweb.qunar.com/';
    var showImg_url = '//qcweb.qunar.com/';
    var sendwap_url = DOWN_URL + 'api/setids';
    ...
    var ChatPage = function() {
        ...
        if(this.useWebSocket && utils.isSupportWebSocket()) {
            bosh_service_url = 'ws:' + DOWN_URL + 'websocket';
        }
    ...
    ChatPage.prototype = {
        ...
        handleImgUrl: function(img) {
            ...
            if (img) {
                if (protocal.indexOf('https:') >= 0) {
                    ...
                    img = img.replace(/^\/\/[^/]+/, '//imgs.qunarzz.com');
                } else if (protocal.indexOf('http:') >= 0) {
                    img = img.replace(/^(http|https):/, '');
                    img = img.replace(/^\/\/[^/]+/, '//img1.qunarzz.com');
                }
                return img;
            } else {
                return "";
                    ...
                    QunarAPI.ready(function() {
                        QunarAPI.chooseImage({
                            ...
                            success: function(result) {
                                ...
                                url = imageUpload_url + "file/v2/upload/img?" + paramLink;

                                //校验上传的图片是否存在
                                //如果图片存在了就不上传了，直接显示为和上传成功的效果
                                var checkFileUrl = imageUpload_url + "/file/v2/inspection/img?" + paramLink;
                                debugger
                        ...
                        add: function(e, data) {
                            data.process().done(function() {
                                ...
                                $.each(data.files, function(index, file) {
                                    ...
                                    var url = imageUpload_url + "file/v2/upload/img?" + paramLink;
        ...
        getShopName: function(shopId) {
            var self = this,
                ...
                url = '//qcweb.qunar.com/api/newapi/domain/get_vcard_info.qunar?u=' + u + '&k=' + k;
            $.ajax({
                url: url,
                type: 'POST',
                dataType: 'json',
                ...
                success: function(res) {
                    if (res.ret && res.data && res.data[0] && res.data[0].users) {
