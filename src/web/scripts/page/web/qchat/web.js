...
$(document).ready(function() {
    ...
    var DOWN_URL = '//qcweb.qunar.com/';

    var bosh_service_url = DOWN_URL + 'http-bind/';
    var http_api_server = DOWN_URL + 'api/';
    var loginDialog = require("./loginDialog.js");

    var QCAdminApis = {
        // rightbar的detail展示数据
        detailUrl: '/api/pdt/productDtl.qunar',
        // 最近咨询
        recentProUrl: '/api/pdt/lastOne.json',
    ...
    var ChatPage = function() {
        ...
        var canUseWebSocket = this.getQueryString('usews') === '1' && utils.isSupportWebSocket();
        if(canUseWebSocket) {
            bosh_service_url = (location.protocol === 'https:' ? 'wss:' : 'ws:') + DOWN_URL + 'websocket';
        }
    ...
    ChatPage.prototype = {
        _getQueryParams: function() {
            ...
            this.bu_name = this.getQueryString('line') || 'dujia';
            // 1 - 售后； 2 - 售前
            this.service_type = this.getQueryString('service_type') || '0';
            this.service_url = this.getQueryString('url') || 'http://qunar.com/';
            // 是否匿名-未登录就是匿名
        ...
        config: function() {
            ...
            // 已登陆或者开启了匿名登陆都直接初始化
            if(loginDialog.isLogin()|| isAnonymous) {
                this.initConverse();
            } else {
        ...
        initEvent: function() {
            ...
            $(document).on("click", "#imageupload", function(e) {
                ...
                var url = DOWN_URL + "/file/v2/upload/img?size=48&u=" + u + "&k=" + k + "&key=test";
                
                //var url = DOWN_URL + "cgi-bin/file_upload.pl?user=" + this.converse.settings.get("myName") + "&tkey=" + this.converse.settings.get("key");
                    ...
                    add: function(e, data) {
                        data.process().done(function() {
                            $.each(data.files, function(index, file) {
                                ...
                                var url = DOWN_URL + "file/v2/upload/img?" + paramLink;
                                ...
                                var checkFileUrl = DOWN_URL + "/file/v2/inspection/img?" + paramLink;
            ...
            $(document).on("click", "#fileupload", function(e) {
                var u = this.converse.settings.get("myName");
                var k = this.converse.settings.get("key");
                var url = DOWN_URL + "/file/v2/upload/file?size=46&u=" + u + "&k=" + k + "&key=1234";
                //var url = DOWN_URL + "cgi-bin/file_upload.pl?user=" + this.converse.settings.get("myName") + "&tkey=" + this.converse.settings.get("key");
                    ...
                    add: function(e, data) {
                        ...
                        data.process().done(function() {
                            $.each(data.files, function(index, file) {
                                ...
                                var sizeMB = utils.bytesToMB(file.size);
                                var paramLink = "name=" + file.name + "&size=" + sizeMB + "&u=" + u + "&k=" + k + "&key=" + key + "&p=qim_web";
                                var url = DOWN_URL + "file/v2/upload/file?" + paramLink;
                                ...
                                //校验上传的文件是否存在
                                //如果文件存在了就不上传了，直接显示为和上传成功的效果
                                var checkFileUrl = DOWN_URL + "/file/v2/inspection/file?" + paramLink;
        ...
        prefixOrderData: function(data) {
            ...
            dataObj.pic = 'https://imgs.qunarzz.com' + data.main_pic;
        ...
        renderDetail: function(id, businessId, userQName) {
            ...
            //     'jijiu': 7
            // };
            var bu_name = this.getQueryString("line") || "dujia"; //业务线名称
        ...
        getAddC2bOrderInfo: function(from, to) {
            ...
            var data = {
                ...
                seatId: seatId || '',
                virtualId: shopId || '',
                seatHost: self.setting.toDomain || 'ejabhost2'
            };
